...
function Base.length(::AeroCoefficients)
    return 3
end

function length(a::AeroCoefficients)
    return 3
end
...
function calculate_coefficients_low_aoa(airfoil::Airfoil, aoa, corrected_lift_slope, aoa_zero_lift)
...
function calculate_coefficients_stall(airfoil::Airfoil, aoa, corrected_lift_slope, aoa_zero_lift, stall_angle_low, stall_angle_high, flap_angle)
...
function calculate_coefficients(airfoil::Airfoil, aoa, corrected_lift_slope, aoa_zero_lift, stall_angle_low, stall_angle_high, flap_angle)
    # low angle of attack
    ...
    stall_angle_high_padded = lerp(15, 5, (rad2deg(flap_angle) + 50) / 100) |> deg2rad
    stall_angle_high_padded = lerp(15, 5, (-rad2deg(flap_angle) + 50) / 100) |> deg2rad
    
    # high angle of attack | full stall
    if aoa < stall_angle_low_padded || aoa > stall_angle_high_padded
    ...
    aero_coefficients_low_aoa = calculate_coefficients_low_aoa(airfoil, aoa, corrected_lift_slope, aoa_zero_lift)
    aero_coefficients_stall = calculate_coefficients_stall(airfoil, aoa, corrected_lift_slope, aoa_zero_lift, stall_angle_low, stall_angle_high, flap_angle)
...
function calculate_forces(airfoil::Airfoil, aircraft_twist::MultiVector, flight_surface_pose::MultiVector, flap_angle::Float64)
    ...
    aero_coefficients = calculate_coefficients(airfoil::Airfoil, aoa, corrected_lift_slope, aoa_zero_lift, stall_angle_low, stall_angle_high, flap_angle)
